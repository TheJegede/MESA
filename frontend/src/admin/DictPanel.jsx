...
import { getDictJobs, getDictJobDownloadUrl, resendDictJob, getDictJobEntries, setBaseline } from '../api/mesa'

function ChangeNoteModal({ note, onClose }) {
  if (!note) return null;
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(33,49,77,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
      <div style={{ background: '#fff', width: '100%', maxWidth: 700, borderRadius: 12, overflow: 'hidden', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04)' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)' }}>
          <h3 style={{ fontFamily: 'Montserrat', fontWeight: 700, fontSize: 18, color: 'var(--dark-blue)', margin: 0 }}>System Change Note</h3>
          <button onClick={onClose} style={{ border: 'none', background: 'transparent', fontSize: 24, cursor: 'pointer', color: 'var(--silver)' }}>×</button>
        </div>
        <div style={{ padding: 24, maxHeight: '70vh', overflowY: 'auto', fontSize: 14, color: '#4A5568', lineHeight: 1.6 }}>
          <div style={{ background: '#F7FAFC', padding: 20, borderRadius: 8, border: '1px solid #E2E8F0', whiteSpace: 'pre-wrap', fontFamily: 'inherit' }}>
            {note}
          </div>
          <div style={{ marginTop: 24, fontSize: 12, color: 'var(--silver)', fontStyle: 'italic' }}>
            Generated by Agent 2 Impact Analysis (local inference).
          </div>
        </div>
        <div style={{ padding: '16px 24px', background: 'var(--surface)', borderTop: '1px solid var(--border)', textAlign: 'right' }}>
          <button onClick={onClose} style={{ background: 'var(--blaster-blue)', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 20px', fontWeight: 700, fontFamily: 'Montserrat', fontSize: 13, cursor: 'pointer' }}>Close</button>
        </div>
      </div>
    </div>
  );
...
function JobHistoryRow({ j, onNoteClick, onBaselineSet }) {
  ...
  const handleSetBaseline = async () => {
    if (!window.confirm(`Set this ${j.system} schema as the baseline for future comparisons?`)) return;
    setBaselining(true);
    try {
      await setBaseline(j.id);
      alert(`${j.system} baseline updated.`);
      onBaselineSet();
    } catch (e) {
  ...
  return (
    <div
      ...
      style={{
        gridTemplateColumns: "80px 1.2fr 100px 140px 90px 140px 160px 180px",
    ...
    >
      ...
      <div className="mono truncate" style={{ fontSize: 12.5, color: "var(--dark-blue)" }} title={j.filename}>
        {j.filename}
      </div>
      <div>
        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--blaster-blue)', textTransform: 'uppercase' }}>{j.system}</span>
      </div>
      <div>
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        {j.status === "completed" ? (
          <>
            <button
              onClick={() => onNoteClick(j.change_note)}
              style={{
                color: "var(--golden-tech)", fontWeight: 700, fontSize: 10,
                fontFamily: "Montserrat", letterSpacing: "0.04em",
                padding: "3px 6px", border: "1px solid var(--golden-tech)", borderRadius: 4,
            ...
            <button
              ...
              disabled={baselining}
              style={{
                color: "var(--mines-green)", fontWeight: 700, fontSize: 10,
                fontFamily: "Montserrat", letterSpacing: "0.04em",
                padding: "3px 6px", border: "1px solid var(--mines-green)", borderRadius: 4,
                background: "transparent", cursor: baselining ? "default" : "pointer",
              }}
            >Baseline</button>
            <a
              ...
              download
              style={{
                color: "var(--blaster-blue)", fontWeight: 700, fontSize: 10,
                fontFamily: "Montserrat", letterSpacing: "0.04em", textDecoration: "none",
                padding: "3px 6px", border: "1px solid var(--blaster-blue)", borderRadius: 4,
              }}
            >CSV</a>
            <button
...
function DictPanel() {
  ...
  return (
    <div className="fade-in" style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      {activeNote && <ChangeNoteModal note={activeNote} onClose={() => setActiveNote(null)} />}
      ...
      >
        <div className="flex items-center justify-between px-5 py-4">
          <div>
            ...
            <div style={{ fontSize: 11.5, color: "var(--silver)", marginTop: 2 }}>
              Recent Agent 2 dictionary jobs · Analysis highlights structural deltas against baselines
            </div>
          </div>
          <span style={{
        ...
        <DPTableHeader
          cols={["Job ID", "Filename", "System", "Triggered By", "Entries", "Status", "Timestamp", "Actions"]}
          gridTemplate="80px 1.2fr 100px 140px 90px 140px 160px 180px"
        />
        <div>
          {jobs.length === 0 ? (
            ...
            jobs.map((j) => (
              <JobHistoryRow
                key={j.id}
                j={j}
                onNoteClick={(note) => setActiveNote(note || "No analysis available for this job.")}
                onBaselineSet={fetchJobs}
              />
            ))
          )}
        </div>

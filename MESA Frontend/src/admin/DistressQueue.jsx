...
function DistressQueue() {
  const [flags, setFlags] = useStateQ(INITIAL_FLAGS);
  const [confirmingId, setConfirmingId] = useStateQ(null);
  const [successMsg, setSuccessMsg] = useStateQ(null);
  const [removingIds, setRemovingIds] = useStateQ([]);
  const [countdown, setCountdown] = useStateQ(60);
  const successTimer = useRefQ(null);
...
window.DistressQueue = DistressQueue;

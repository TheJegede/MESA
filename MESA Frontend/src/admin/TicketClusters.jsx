...
function TicketClusters() {
  const [threshold, setThreshold] = useStateT(10);
  const [activeFilter, setActiveFilter] = useStateT("All");

  const filtered = useMemoT(() => (
    activeFilter === "All" ? TC_ALL_TICKETS : TC_ALL_TICKETS.filter((t) => t.system === activeFilter)
  ), [activeFilter]);

  const aboveCount = TC_CLUSTERS.filter((c) => c.count >= threshold).length;
  ...
  return (
    <div className="fade-in" style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      >
        ...
        />
        <div>
          {TC_CLUSTERS.map((c) => <ClusterTableRow key={c.id} c={c} threshold={threshold} />)}
        </div>
...
window.TicketClusters = TicketClusters;
